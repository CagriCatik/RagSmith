...
from pdf_md_rag.backends import registry
from pdf_md_rag.config import AppConfig, BackendConfig

LOGGER = logging.getLogger("pdf_md_rag.ui")
    ...
    def run(self) -> None:  # pragma: no cover - UI thread
        try:
            ...
            for idx, file in enumerate(self.files, 1):
                self.app.convert_and_write([file], self.output_dir, split_sections=self.split, reflow=self.reflow)
